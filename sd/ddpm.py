...
class DDPMSampler:
    ...
    def _get_previous_timestpe(self, timestep: int) -> int:
    ...
    def _get_variance(self, timestep: int) -> torch.Tensor:
        ...
        variance = torch.clamp(variance, min=1e-20)  # ensure it doesnt reach 0
    ...
    def add_noise(
        self, original_sample: torch.Tensor, timesteps: torch.Tensor
    ) -> torch.FloatTensor:
        alpha_cumprod = self.alpha_cumprod.to(
            device=original_sample.device, dtype=original_sample.dtype
        )
        ...
        stdev = sqrt_one_minus_alpha_prod

        noisy_sample = mean + stdev * noise
